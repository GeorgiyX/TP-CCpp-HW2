...
void fork_work(int *arr, size_t array_size, int pipe_fd[2]) {
    ...
    do {
        if (get_check_sum_private(arr, array_size, &check_sum)) { break; }
        if (write_all(pipe_fd[WRITE_INDEX], (char *) &check_sum, sizeof(check_sum))) { break; }
...
int get_check_sum(int *arr, size_t array_size, int *check_sum) {
    ...
    do {
        ...
        int *check_sums = calloc(num_of_forks, sizeof(int));
        if (!check_sums) { break; }
        if (read_all(pipe_fd[READ_INDEX], (char *) check_sums, num_of_forks * sizeof(int))) {
            free(check_sums);
            break;
        }
